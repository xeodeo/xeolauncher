...
{
    qCDebug(logCat) << "[DBG] downloadFinished:" << m_url.toString() << "state=" << (int)m_state;
    ...
    qCDebug(logCat) << "[DBG] downloadFinished: reading remaining data";
    ...
    qCDebug(logCat) << "[DBG] downloadFinished: calling sink finalize";
    m_state = m_sink->finalize(*m_reply.get());
    qCDebug(logCat) << "[DBG] downloadFinished: finalize done, state=" << (int)m_state;
    if (m_state != State::Succeeded) {
    ...
    qCDebug(logCat) << "[DBG] downloadFinished: emitting succeeded";
    emit succeeded();
    qCDebug(logCat) << "[DBG] downloadFinished: emitting finished";
    emit finished();
    qCDebug(logCat) << "[DBG] downloadFinished: done";
}
