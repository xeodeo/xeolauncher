...
{
    qCDebug(taskNetLogC) << "[DBG] FileSink::finalize start, filename=" << m_filename;
    bool gotFile = false;
    ...
    qCDebug(taskNetLogC) << "[DBG] FileSink::finalize statusCode=" << statusCode << "gotFile=" << gotFile << "wroteAnyData=" << m_wroteAnyData;
    ...
    if (gotFile || m_wroteAnyData) {
        ...
        qCDebug(taskNetLogC) << "[DBG] FileSink::finalize calling finalizeAllValidators";
        if (!finalizeAllValidators(reply)) {
            m_fail_reason = "Failed to finalize validators";
            return Task::State::Failed;
        }
        qCDebug(taskNetLogC) << "[DBG] FileSink::finalize validators OK, committing file";
        ...
        qCDebug(taskNetLogC) << "[DBG] FileSink::finalize commit OK";
    }

    // then get rid of the save file
    m_output_file.reset();
    qCDebug(taskNetLogC) << "[DBG] FileSink::finalize done";
