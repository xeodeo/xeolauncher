...
{
    qDebug() << "[DBG] infoRequestSucceeded: start";
    auto currentPack = data(index, Qt::UserRole).value<ModPlatform::IndexedPack::Ptr>();

    // Check if the index is still valid for this resource or not
    if (!currentPack || pack->addonId != currentPack->addonId) {
        qDebug() << "[DBG] infoRequestSucceeded: early return (currentPack null or mismatch)";
        return;
    }
    qDebug() << "[DBG] infoRequestSucceeded: proceeding";

    // Cache info :^)
    QVariant newPack;
    newPack.setValue(pack);
    qDebug() << "[DBG] infoRequestSucceeded: before setData";
    if (!setData(index, newPack, Qt::UserRole)) {
        qWarning() << "Failed to cache resource info!";
        return;
    }
    qDebug() << "[DBG] infoRequestSucceeded: after setData, before emit";
    emit projectInfoUpdated(index);
    qDebug() << "[DBG] infoRequestSucceeded: after emit";
}
