...
{
    qDebug() << "[DBG] updateUi: start";
    if (index != m_ui->packView->currentIndex()) {
        qDebug() << "[DBG] updateUi: index mismatch, returning";
        return;
    }

    auto currentPack = getCurrentPack();
    if (!currentPack) {
        qDebug() << "[DBG] updateUi: no currentPack";
        m_ui->packDescription->setHtml({});
        m_ui->packDescription->flush();
        return;
    }
    qDebug() << "[DBG] updateUi: building text, extraDataLoaded=" << currentPack->extraDataLoaded;
    QString text = "";
    ...
    qDebug() << "[DBG] updateUi: calling markdownToHTML, body size=" << currentPack->extraData.body.size();
    auto bodyHtml = currentPack->extraData.body.isEmpty() ? currentPack->description : markdownToHTML(currentPack->extraData.body);
    qDebug() << "[DBG] updateUi: calling setHtml";
    m_ui->packDescription->setHtml(StringUtils::htmlListPatch(text + bodyHtml));
    qDebug() << "[DBG] updateUi: calling flush";
    m_ui->packDescription->flush();
    qDebug() << "[DBG] updateUi: done";
}
