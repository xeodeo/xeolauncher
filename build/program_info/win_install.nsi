...
Name "Xeo Launcher"
InstallDir "$LOCALAPPDATA\Programs\XeoLauncher"
InstallDirRegKey HKCU "Software\XeoLauncher" "InstallDir"
RequestExecutionLevel user
OutFile "../XeoLauncher-Setup.exe"

!define MUI_ICON "../program_info/xeolauncher.ico"

!define UNINST_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\XeoLauncher"
...
!define MUI_FINISHPAGE_RUN "$InstDir\xeolauncher.exe"
...
VIFileVersion "12.0.0.0"
VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "Xeo Launcher"
VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "Xeo Launcher Installer"
VIAddVersionKey /LANG=${LANG_ENGLISH} "LegalCopyright" "© 2026 XeoLauncher Contributors\n© 2022-2026 Prism Launcher Contributors\n© 2012-2021 MultiMC Contributors"
VIAddVersionKey /LANG=${LANG_ENGLISH} "FileVersion" "12.0.0.0"
VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductVersion" "12.0.0.0"
...
Section "Xeo Launcher"

  SectionIn RO

  nsExec::Exec /TIMEOUT=2000 'TaskKill /IM xeolauncher.exe /F'

  SetOutPath $INSTDIR

  File "xeolauncher.exe"
  File "xeolauncher_filelink.exe"
  File "xeolauncher_updater.exe"
  File "qt.conf"
  File "qtlogging.ini"
  File *.dll
  File /r "iconengines"
  ...
  WriteRegStr HKCU Software\XeoLauncher "InstallDir" "$INSTDIR"

  ; Write the URL Handler into registry for curseforge
  WriteRegStr HKCU Software\Classes\curseforge "URL Protocol" ""
  WriteRegStr HKCU Software\Classes\curseforge\shell\open\command "" '"$INSTDIR\xeolauncher.exe" "%1"'

  ; Write the URL Handler into registry for prismlauncher
  WriteRegStr HKCU Software\Classes\xeolauncher "URL Protocol" ""
  WriteRegStr HKCU Software\Classes\xeolauncher\shell\open\command "" '"$INSTDIR\xeolauncher.exe" "%1"'

  ; Write the URL Handler into registry for prismlauncher import
  WriteRegStr HKCU Software\Classes\prismlauncher "URL Protocol" ""
  WriteRegStr HKCU Software\Classes\prismlauncher\shell\open\command "" '"$INSTDIR\xeolauncher.exe" "%1"'
  ...
  ${If} ${Errors}
    WriteRegStr HKCU "${UNINST_KEY}" "DisplayName" "Xeo Launcher"
    WriteRegStr HKCU "${UNINST_KEY}" "DisplayIcon" "$INSTDIR\xeolauncher.exe"
    ...
    WriteRegStr HKCU "${UNINST_KEY}" "Publisher" "Xeo Launcher Contributors"
    WriteRegStr HKCU "${UNINST_KEY}" "Version" "12.0.0.0"
    WriteRegStr HKCU "${UNINST_KEY}" "DisplayVersion" "12.0.0"
    WriteRegStr HKCU "${UNINST_KEY}" "VersionMajor" "12"
...
Section "Start Menu Shortcut" SM_SHORTCUTS
  ...
  CreateShortcut "$SMPROGRAMS\Xeo Launcher.lnk" "$INSTDIR\xeolauncher.exe" "" "$INSTDIR\xeolauncher.exe" 0

SectionEnd

Section /o "Desktop Shortcut" DESKTOP_SHORTCUTS

  CreateShortcut "$DESKTOP\Xeo Launcher.lnk" "$INSTDIR\xeolauncher.exe" "" "$INSTDIR\xeolauncher.exe" 0

SectionEnd


!define APPID "XeoLauncher.App"
!define APPEXE "xeolauncher.exe"
!define APPICON "$INSTDIR\${APPEXE},0"
!define APPDESCRIPTION "Xeo Launcher"
!define APPNAME "Xeo Launcher"
!define APPCMDTEXT "Xeo Launcher"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\XeoLauncher"
  DeleteRegKey HKCU SOFTWARE\XeoLauncher

  Delete $INSTDIR\xeolauncher.exe
  Delete $INSTDIR\xeolauncher_filelink.exe
  Delete $INSTDIR\xeolauncher_updater.exe
  ...
  Delete "$SMPROGRAMS\Xeo Launcher.lnk"
  Delete "$DESKTOP\Xeo Launcher.lnk"
